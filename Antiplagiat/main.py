...
class Window(QMainWindow):
    ...
    def onClick(self):
        leftWords = [elem.strip()
                     for elem in self.leftTextField.toPlainText().split('\n') if elem]
        rightWords = [
            elem.strip() for elem in self.rightTextField.toPlainText().split('\n') if elem]

        possibilities = list()
        for line1, line2 in zip(leftWords, rightWords):
            forwardScan = list(
                map(lambda ltrs: ltrs[0] == ltrs[1], zip_longest(line1, line2)))
            backwardScan = list(
                map(lambda ltrs: ltrs[0] == ltrs[1], zip_longest(line1[::-1], line2[::-1])))
